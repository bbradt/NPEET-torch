...
def mi(x, y, z=None, k=3, base=2, alpha=0):
    ...
    if z is not None:
        ...
        points.append(z)
    points = torch.hstack(points)
    # Find nearest neighbors in joint space, p=inf means max-norm
    tree = build_tree(points)
    dvec = query_neighbors(tree, points, k)
    if z is None:
        a, b, c, d = avgdigamma(x, dvec), avgdigamma(
            y, dvec), digamma(k), digamma(len(x)) # TODO
        if alpha > 0:
            d += lnc_correction(tree, points, k, alpha) # TODO
    else:
        xz = np.c_[x, z]
        yz = np.c_[y, z]
...
def add_noise(x, intens=1e-10):
    ...
    return x + intens * torch.rand(x.shape).to(x.device)


def query_neighbors(tree, x, k):
    return tree.query(x.detach().cpu().numpy(), k=k + 1)[0][:, k]


def count_neighbors(tree, x, r):
    return tree.query_radius(x.detach().cpu().numpy(), r, count_only=True)
...
def avgdigamma(points, dvec):
    ...
    return torch.mean(torch.from_numpy(digamma(num_points)).to(points.device))


def build_tree(points):
    if points.shape[1] >= 20:
        return BallTree(points.detach().cpu().numpy(), metric='chebyshev')
    return KDTree(points.detach().cpu().numpy(), metric='chebyshev')
    #return KNN()
...
if __name__ == "__main__":
    ...
    for N in Ns:
        for M in Ms:
            ...
            for i in range(10):
                ...
                start = time.time()
                MIc = mi(x, y, base=2, alpha=0)
                rt_cpu = time.time() - start

                start = time.time()
                MId = mi(xt, yt, base=2, alpha=0)
                rt = time.time() - start
                rows.append(dict(device="cpu", runtime=rt_cpu, N=N, M=M, MI=MIc))
                print(rows[-1])
                rows.append(dict(device="cuda", runtime=rt, N=N, M=M, MI=MId ))
                print(rows[-1])
    ...
    #plt.xscale("log")
    #plt.yscale("log")
    sb.lineplot(x="N", y="runtime", hue="device", data=df)
